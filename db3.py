...
def generate_sample_data(db):
    product_names = ["콜라", "사이다", "주스", "우유", "커피", 
                     "차", "스낵", "과자", "아이스크림", "샌드위치"]
    for i in range(100):
        product_name = random.choice(product_names) + f" {i+1}"
        price = round(random.uniform(1, 50), 2)  # 1 ~ 50 사이의 랜덤 가격
...
if __name__ == "__main__":
    ...
    # 제품 업데이트 예시
    db.update_product(1, product_name="업데이트된 제품", price=29.99)

    # 특정 제품 조회
    product = db.select_product(1)
    print("업데이트된 제품:", product)
